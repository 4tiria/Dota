...
const HeroList = () => {
    ...
    function getListOfElements(heroes: Hero[]): JSX.Element[] {
        let result = heroes.map((h) => <HeroInList hero={h} key={h.id} />)
    ...
    function splitAndRenderAllRows(columns: number) {
        if (list?.length > 0) {
            ...
            return splitList.map((row, index) => (
                <div key={index}>{renderRow(columns, row)}</div>
            ))
        }
    }
    ...
    function renderRow(columns: number, array: JSX.Element[]) {
        ...
        if (difference > 0) {
            for (let i = 0; i < difference; i++) {
                array.push(<HeroInList hero={null} key={generateUniqueID()} />)
            }
        }
    ...
    return (
        <Paper sx={{ paddingBottom: 20 }} variant="elevation" square={true}>
            <FilterPanel
                callBackFunction={(heroFilterModel) => {
            ...
            <div className="hero-list">
                <div>{splitAndRenderAllRows(4)}</div>
            </div>
        </Paper>
    )
}
