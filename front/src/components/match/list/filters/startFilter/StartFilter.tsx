...
const StartFilter = () => {
    ...
    const [value, setValue] = useState<ValueMs[]>([
        valueArray[valueArray.length - 4],
    ...
    useEffect(() => {
        ...
        const timeOutId = setTimeout(
            () =>
                dispatch(
                    matchFilterSlice.actions.setStart(value.map((x) => x.ms))
                ),
            waitForInputMilliseconds
        )
        return () => clearTimeout(timeOutId)
    }, [dispatch, enableFilter, value])

    useEffect(() => {
        dispatch(
            matchFilterSlice.actions.setStart(
                enableFilter ? value.map((x) => x.ms) : [null, null]
            )
        )
    }, [dispatch, enableFilter, value])
        ...
        <div>
            <div className="d-flex justify-content-between">
                <div>
                    ...
                    <div>{`${value[0].label} - ${value[1].label} ago`}</div>
                </div>
                <div>
                    <Checkbox
                        color="secondary"
